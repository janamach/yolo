...
hb = ax.hexbin(birds_df['X1'], birds_df['Y1'], gridsize=80, cmap='inferno', bins='log')
...
plt.savefig(output_path + '/heatmap_' + basename + '.png')
plt.close()
...
if food:
    ...
    plt.ylim(0, 1296)
    plt.gca().invert_yaxis()
    plt.savefig(output_path + '/heatmap_' + basename + '_feeding.png')
    plt.close()
    ...
    feeding_df2 = feeding_df[::30].reset_index(drop=True)
    # Save as csv file
    feeding_df2.to_csv(output_path + '/feeding_count_' + basename + '.csv', index=True)
